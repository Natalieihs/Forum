...
layui.define(['layer', 'form'], function (exports) {
        ...
        , Edit = function () {
            ...
            that.config = {
                ...
                tool: [
                    ...
                    , 'left', 'center', 'right'
                    , '|'
                    , 'link', 'unlink', 'face', 'coludimage','code'
                ]
                , hideTool: []
                , height: 280 //默认高
            };
    ...
    Edit.prototype.setCursorText = function (index, content) {
        var iframeWin = getWin(index);
        if (!iframeWin[0]) return;
        try {
            var range = Range(iframeWin[0].document);
            insertInline.call(iframeWin[0], 'div', {
                text: content
            }, range);
        }
        catch (err) {
            this.setContent(index, '<div>' + content+'</div>',true);
        }
        //layedit.sync(index);
    };
    ...
    var setIframe = function (editor, textArea, set) {
        ...
        , tools = {
            ...
            , coludimage: '<i class="layui-icon layedit-tool-image" title="图片" layedit-event="coludimage">&#xe64a;</i>'
